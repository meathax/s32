...
always_comb begin
    ...
    case (state)
    STATE_OPCODE: begin
        insn_complete = (!fifo_empty && !is_prefix(opcode_data) &&
            !insn_has_modrm(opcode_data) &&
            insn_immed_count(opcode_data, 3'b0) == 2'b00) ||
            instruction.invalid;
        next_state = fifo_empty ? STATE_OPCODE :
            is_prefix(opcode_data) ? STATE_OPCODE :
            insn_has_modrm(opcode_data) ? STATE_MODRM :
            insn_immed_count(opcode_data, 3'b0) != 2'b00 ? STATE_IMMED1 :
    ...
    if (reset) begin
        ...
        STATE_OPCODE: begin
            if (!fifo_empty) begin
                ...
                default: begin
                    instruction.opcode <= opcode_data;
                    instruction.has_modrm <= insn_has_modrm(opcode_data);
                end
                endcase
                ...
                if (next_state == STATE_IMMED1) begin
                    immed_start <= 1'b1;
                    immed_is_8bit <= insn_immed_is_8bit(opcode_data,
                                                        3'b0, 1'b0);
                end
            end
        end

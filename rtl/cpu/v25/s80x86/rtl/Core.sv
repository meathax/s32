...
module Core(input logic clk,
            ...
            // Optional opcode-only translation. The raw decoder byte is
            // exposed on opcode_xlat_in and opcode_xlat_out is consumed only
            // while the instruction decoder is in its opcode state.
            output logic [7:0] opcode_xlat_in,
            input logic [7:0] opcode_xlat_out,
...
wire fifo_full;
wire fifo_reset;
wire [7:0] decoded_opcode = opcode_xlat_en ? opcode_xlat_out : fifo_rd_data;
assign opcode_xlat_in = fifo_rd_data;

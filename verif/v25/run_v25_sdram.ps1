...
if (-not (Test-Path -LiteralPath $msysBash -PathType Leaf)) {
    throw 'MSYS2 bash is required for the native Windows V25 SDRAM runner.'
}
...
try {
    ...
    $env:KEEP_BUILD = '1'
    $script = ($repoRoot -replace '\\', '/') + '/verif/v25/run_v25_sdram.sh'

    [string[]]$buildOutput = & $msysBash -lc $script 2>&1
    $buildExitCode = $LASTEXITCODE
    $buildOutput | ForEach-Object { Write-Output $_ }
    if ($buildExitCode -ne 0) {
        throw "V25 SDRAM build failed with exit code $buildExitCode."
    }

    $exePrefix = 'V25_SDRAM EXE: '
    $dirPrefix = 'V25_SDRAM BUILD DIR: '
    ...
    if (-not $exeLine -or -not $dirLine) {
        throw 'V25 SDRAM build did not report its executable and build directory.'
    }
    ...
    $expectedExe = Join-Path $buildDir 'obj_dir\Vtb_core_v25sdram.exe'
    if (-not $buildDir.StartsWith('R:\Verilator\', [StringComparison]::OrdinalIgnoreCase) -or
        ...
        throw "Refusing an unexpected V25 SDRAM build path: $buildDir"
    }
    $validatedBuildDir = $true
    if (-not (Test-Path -LiteralPath $firmwareExe -PathType Leaf)) {
        throw "V25 SDRAM executable was not produced: $firmwareExe"
    }
    ...
    if ($runExitCode -ne 0) {
        throw "V25 SDRAM simulation failed with exit code $runExitCode."
    }
    if (-not ($runOutput | Where-Object { $_.Contains('V25 SDRAM INTEGRATION PASS') })) {
        throw 'V25 SDRAM simulation completion marker is missing.'
    }
    Write-Output 'V25_SDRAM RUNNER: PASS'
}
finally {
    $env:PATH = $savedPath
    ...
    elseif ($buildDir -and (Test-Path -LiteralPath $buildDir)) {
        Write-Warning "Retained unvalidated V25 SDRAM build directory: $buildDir"
    }
}

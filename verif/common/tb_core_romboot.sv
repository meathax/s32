...
// Mixer per-pixel probe: capture the winning layer, palette index, palette word
// and final RGB for a short run of pixels on one scanline, so a colour that
// appears in no palette entry can be traced to the layer that produced it.
localparam int MIXPROBE_MAX = 24;
integer n_mixprobe = 0, mixprobe_i;
integer mixprobe_frame = -1, mixprobe_row = -1, mixprobe_col = -1;
integer mixprobe_x   [0:MIXPROBE_MAX-1];
integer mixprobe_y   [0:MIXPROBE_MAX-1];
integer mixprobe_sel [0:MIXPROBE_MAX-1];
reg [15:0] mixprobe_idx [0:MIXPROBE_MAX-1];
reg [15:0] mixprobe_pal [0:MIXPROBE_MAX-1];
reg [23:0] mixprobe_rgb [0:MIXPROBE_MAX-1];
reg        mixprobe_act [0:MIXPROBE_MAX-1];
reg        mixprobe_bl  [0:MIXPROBE_MAX-1];
reg        mixprobe_sh  [0:MIXPROBE_MAX-1];
reg [15:0] mixprobe_itx [0:MIXPROBE_MAX-1];
reg [15:0] mixprobe_i0  [0:MIXPROBE_MAX-1];
reg [15:0] mixprobe_i1  [0:MIXPROBE_MAX-1];
reg [15:0] mixprobe_i2  [0:MIXPROBE_MAX-1];
reg [15:0] mixprobe_ptx [0:MIXPROBE_MAX-1];

// Reconstruct a palette RAM word from the bit-sliced plane RAMs, so the RAM the
// mixer actually reads can be compared against the native-format sim shadow the
// dumps use.
reg [15:0] palram_w;
task automatic read_palram(input integer row, input integer bank);
    begin
        palram_w[ 0] = bank ? core.pal0.g_pal_bit[ 0].bank1.mem[row] : core.pal0.g_pal_bit[ 0].bank0.mem[row];
        palram_w[ 1] = bank ? core.pal0.g_pal_bit[ 1].bank1.mem[row] : core.pal0.g_pal_bit[ 1].bank0.mem[row];
        palram_w[ 2] = bank ? core.pal0.g_pal_bit[ 2].bank1.mem[row] : core.pal0.g_pal_bit[ 2].bank0.mem[row];
        palram_w[ 3] = bank ? core.pal0.g_pal_bit[ 3].bank1.mem[row] : core.pal0.g_pal_bit[ 3].bank0.mem[row];
        palram_w[ 4] = bank ? core.pal0.g_pal_bit[ 4].bank1.mem[row] : core.pal0.g_pal_bit[ 4].bank0.mem[row];
        palram_w[ 5] = bank ? core.pal0.g_pal_bit[ 5].bank1.mem[row] : core.pal0.g_pal_bit[ 5].bank0.mem[row];
        palram_w[ 6] = bank ? core.pal0.g_pal_bit[ 6].bank1.mem[row] : core.pal0.g_pal_bit[ 6].bank0.mem[row];
        palram_w[ 7] = bank ? core.pal0.g_pal_bit[ 7].bank1.mem[row] : core.pal0.g_pal_bit[ 7].bank0.mem[row];
        palram_w[ 8] = bank ? core.pal0.g_pal_bit[ 8].bank1.mem[row] : core.pal0.g_pal_bit[ 8].bank0.mem[row];
        palram_w[ 9] = bank ? core.pal0.g_pal_bit[ 9].bank1.mem[row] : core.pal0.g_pal_bit[ 9].bank0.mem[row];
        palram_w[10] = bank ? core.pal0.g_pal_bit[10].bank1.mem[row] : core.pal0.g_pal_bit[10].bank0.mem[row];
        palram_w[11] = bank ? core.pal0.g_pal_bit[11].bank1.mem[row] : core.pal0.g_pal_bit[11].bank0.mem[row];
        palram_w[12] = bank ? core.pal0.g_pal_bit[12].bank1.mem[row] : core.pal0.g_pal_bit[12].bank0.mem[row];
        palram_w[13] = bank ? core.pal0.g_pal_bit[13].bank1.mem[row] : core.pal0.g_pal_bit[13].bank0.mem[row];
        palram_w[14] = bank ? core.pal0.g_pal_bit[14].bank1.mem[row] : core.pal0.g_pal_bit[14].bank0.mem[row];
        palram_w[15] = bank ? core.pal0.g_pal_bit[15].bank1.mem[row] : core.pal0.g_pal_bit[15].bank0.mem[row];
    end
endtask

// Sample at the mixer's own output phase (ph==6, when rgb is formed) on the
// probe row, starting at the probe column.
always @(posedge clk_ram) begin
    if (mixprobe_frame >= 0 && cur_frame == mixprobe_frame
        && core.mix0.ph == 4'd6
        && core.mix0.disp_y == mixprobe_row[8:0]
        && core.mix0.disp_x >= mixprobe_col[8:0]
        && n_mixprobe < MIXPROBE_MAX) begin
        mixprobe_x  [n_mixprobe] = core.mix0.disp_x;
        mixprobe_y  [n_mixprobe] = core.mix0.disp_y;
        mixprobe_sel[n_mixprobe] = core.mix0.bestsel_hold;
        mixprobe_idx[n_mixprobe] = {2'b0, core.mix0.pal_addr_r};
        mixprobe_pal[n_mixprobe] = core.mix0.first_pal;
        mixprobe_act[n_mixprobe] = core.mix0.act_hold;
        mixprobe_bl [n_mixprobe] = core.mix0.blend_hold;
        mixprobe_sh [n_mixprobe] = core.mix0.shadow_hold;
        mixprobe_rgb[n_mixprobe] = core.mix0.rgb;
        mixprobe_itx[n_mixprobe] = core.mix0.idx_text_s;
        mixprobe_i0 [n_mixprobe] = core.mix0.idx_nbg0_s;
        mixprobe_i1 [n_mixprobe] = core.mix0.idx_nbg1_s;
        mixprobe_i2 [n_mixprobe] = core.mix0.idx_nbg2_s;
        mixprobe_ptx[n_mixprobe] = core.mix0.px_text;
        n_mixprobe = n_mixprobe + 1;
    end
end
...
initial begin
    ...
    if (!$value$plusargs("MIXPROBEAT=%d", mixprobe_frame)) mixprobe_frame = -1;
    if (!$value$plusargs("MIXPROBEY=%d", mixprobe_row)) mixprobe_row = 35;
    if (!$value$plusargs("MIXPROBEX=%d", mixprobe_col)) mixprobe_col = 110;
...
initial begin
    ...
    for (f = 0; f < frames; f = f + 1) begin
        ...
        $display("   mreg20: %04x %04x %04x %04x %04x %04x %04x %04x",
            core.mix0.mreg[6'h10], core.mix0.mreg[6'h11], core.mix0.mreg[6'h12],
            core.mix0.mreg[6'h13], core.mix0.mreg[6'h14], core.mix0.mreg[6'h15],
            core.mix0.mreg[6'h16], core.mix0.mreg[6'h17]);
        $display("   mreg30: %04x %04x %04x %04x %04x %04x %04x %04x",
            core.mix0.mreg[6'h18], core.mix0.mreg[6'h19], core.mix0.mreg[6'h1a],
            core.mix0.mreg[6'h1b], core.mix0.mreg[6'h1c], core.mix0.mreg[6'h1d],
            core.mix0.mreg[6'h1e], core.mix0.mreg[6'h1f]);
        $display("   mreg40: %04x %04x %04x %04x %04x %04x %04x %04x",
            core.mix0.mreg[6'h20], core.mix0.mreg[6'h21], core.mix0.mreg[6'h22],
            core.mix0.mreg[6'h23], core.mix0.mreg[6'h24], core.mix0.mreg[6'h25],
            core.mix0.mreg[6'h26], core.mix0.mreg[6'h27]);
        $display("   mixbg: bg_ctrl=%04x bg_pen=%04x coff0=%05x coff1=%05x r3e_f=%04x",
            core.mix_bg_ctrl, core.mix0.bg_pen, core.mix0.coloroffs_bank0,
            core.mix0.coloroffs_bank1, core.mix0.r3e_f);
        read_palram(14'h0a00, 0); $display("   palram[0a00] b0=%04x shadow=%04x",
            palram_w, core.pal0.sim_shadow[14'h0a00]);
        read_palram(14'h0a00, 1); $display("   palram[0a00] b1=%04x shadow2a00=%04x",
            palram_w, core.pal0.sim_shadow[14'h2a00]);
        read_palram(14'h08ad, 0); $display("   palram[08ad] b0=%04x shadow=%04x",
            palram_w, core.pal0.sim_shadow[14'h08ad]);
        read_palram(14'h1cd7, 0); $display("   palram[1cd7] b0=%04x shadow=%04x",
            palram_w, core.pal0.sim_shadow[14'h1cd7]);
        $display("   mixprobe: n=%0d", n_mixprobe);
        for (mixprobe_i = 0; mixprobe_i < n_mixprobe; mixprobe_i = mixprobe_i + 1)
            $display("     x=%0d y=%0d sel=%0d idx=%04x pal=%04x act=%b blend=%b shadow=%b rgb=%06x",
                mixprobe_x[mixprobe_i], mixprobe_y[mixprobe_i],
                mixprobe_sel[mixprobe_i], mixprobe_idx[mixprobe_i],
                mixprobe_pal[mixprobe_i], mixprobe_act[mixprobe_i],
                mixprobe_bl[mixprobe_i], mixprobe_sh[mixprobe_i],
                mixprobe_rgb[mixprobe_i]);
        for (mixprobe_i = 0; mixprobe_i < n_mixprobe; mixprobe_i = mixprobe_i + 1)
            $display("     x=%0d itx=%04x i0=%04x i1=%04x i2=%04x pxt=%04x",
                mixprobe_x[mixprobe_i], mixprobe_itx[mixprobe_i],
                mixprobe_i0[mixprobe_i], mixprobe_i1[mixprobe_i],
                mixprobe_i2[mixprobe_i], mixprobe_ptx[mixprobe_i]);

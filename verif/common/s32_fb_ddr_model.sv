...
module s32_fb_ddr_model #(
    parameter integer DEADLINE_CYCLES = 8192
)(
    input             clk,
    input             rst,
...
reg rd_ack_d;
reg [31:0] wr_wait, rd_wait, er_wait;
always @(posedge clk) begin
    if (rst) begin
        rd_ack_d <= 0;
        line_acks <= 0;
        wr_wait <= 0; rd_wait <= 0; er_wait <= 0;
        max_wr_wait <= 0; max_rd_wait <= 0; max_er_wait <= 0;
        deadline_fail <= 0;
    end
    else begin
        rd_ack_d <= rd_ack;
        if (rd_ack && !rd_ack_d) line_acks <= line_acks + 1'd1;

        if (wr_busy) begin
            wr_wait <= wr_wait + 1'd1;
            if (wr_wait + 1'd1 > max_wr_wait) max_wr_wait <= wr_wait + 1'd1;
            if (wr_wait + 1'd1 >= DEADLINE_CYCLES) begin
                deadline_fail <= 1'b1;
        ...
        if (rd_req && !rd_ack) begin
            rd_wait <= rd_wait + 1'd1;
            if (rd_wait + 1'd1 > max_rd_wait) max_rd_wait <= rd_wait + 1'd1;
            if (rd_wait + 1'd1 >= DEADLINE_CYCLES) begin
                deadline_fail <= 1'b1;
                $fatal(1, "GA2 framebuffer line read exceeded %0d clocks",
                       DEADLINE_CYCLES);
            end
        end
        else rd_wait <= 0;
        ...
        if (er_req && !er_ack) begin
            ...
            if (er_wait + 1'd1 >= DEADLINE_CYCLES) begin
                deadline_fail <= 1'b1;
